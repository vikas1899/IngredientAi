...
const AnalysisDetails = () => {
  ...
  const createPdfDocument = (analysis, parsedAnalysis) => {
    ...
    doc.text("Ingredient Analysis Report", pageWidth / 2, 35, {
      align: "center",
    });
    cursorY = 80;
    ...
    const summaryText =
      parsedAnalysis.analysis_summary?.detailed_explanation ||
  ...
  const handleDownload = () => {
    ...
    doc.save(
      `analysis-${analysis.id}-${new Date(analysis.timestamp).toISOString().split("T")[0]}.pdf`,
    );
  };
  ...
  const handleShare = async () => {
    try {
      ...
      const file = new File([pdfBlob], "ingredient-analysis.pdf", {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        alert(
          "Sharing is not supported on this device/browser. The analysis URL has been copied to clipboard.",
        );
      }
    } catch (error) {
  ...
  return (
    <>
      <Navbar />
      <div
        className={`min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 ${showDeleteConfirm ? "filter blur-sm pointer-events-none select-none" : ""}`}
      >
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div className="flex items-start gap-6">
                  <div
                    className={`w-20 h-20 bg-gradient-to-r ${statusInfo.gradientClass} rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg`}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-6 text-gray-600 mb-4">
                      ...
                      {parsedAnalysis?.analysis_summary?.concern_count !==
                        undefined && (
                        <div className="flex items-center">
                    ...
                    <div
                      className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border ${statusInfo.bgColor}`}
                    >
                      {statusInfo.icon}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-8">
            ...
            <div className="xl:col-span-2 space-y-8">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-amber-700">
                            {parsedAnalysis.analysis_summary.concern_count || 0}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-lg font-bold text-blue-700 capitalize">
                            {parsedAnalysis.recommendation?.verdict || "N/A"}
                          </p>
                        </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-700 leading-relaxed">
                          {parsedAnalysis.analysis_summary.main_verdict}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {/* Ingredient Groups */}
              {parsedAnalysis?.ingredient_groups &&
                parsedAnalysis.ingredient_groups.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      {parsedAnalysis.ingredient_groups.map((group, index) => (
                        ...
                        >
                          <button
                            ...
                            onClick={() =>
                              setOpenGroup(openGroup === index ? -1 : index)
                            }
                          >
                            <div className="flex items-center gap-3">
                          ...
                          {openGroup === index && (
                            <div className="divide-y divide-gray-100">
                              {group.ingredients.map((ingredient, i) => {
                                const statusInfo = getIngredientStatusInfo(
                                  ingredient.status,
                                );
                                return (
                                  <div
                                    key={i}
                                    className={`p-6 ${statusInfo.bg} border-l-4 ${statusInfo.bg.includes("emerald") ? "border-l-emerald-400" : statusInfo.bg.includes("amber") ? "border-l-amber-400" : statusInfo.bg.includes("red") ? "border-l-red-400" : "border-l-gray-400"}`}
                                  >
                                    <div className="flex items-start justify-between mb-3">
                                      <div className="flex items-center gap-3">
                                        {statusInfo.icon}
                                        <h5
                                          className={`font-bold text-lg ${statusInfo.text}`}
                                        >
                                          {ingredient.name}
                                        </h5>
                                      </div>
                                      <span
                                        className={`px-3 py-1 text-xs font-semibold rounded-full border ${statusInfo.badge}`}
                                      >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                      <div>
                                        <p
                                          className={`text-sm font-semibold ${statusInfo.text} mb-1`}
                                        >
                                          Purpose:
                                        </p>
                                        <p
                                          className={`text-sm ${statusInfo.text} opacity-90`}
                                        >
                                          {ingredient.purpose ||
                                            "Not specified"}
                                        </p>
                                      ...
                                      <div>
                                        ...
                                        <p
                                          className={`text-sm ${statusInfo.text} opacity-90 capitalize`}
                                        >
                                    ...
                                    <div className="mb-3">
                                      ...
                                      <p
                                        className={`text-sm ${statusInfo.text} opacity-90 leading-relaxed`}
                                      >
                                        {ingredient.quick_summary ||
                                          "No summary available"}
                                      </p>
                                    </div>
              ...
              {/* Health Alerts */}
              {parsedAnalysis?.health_alerts &&
                parsedAnalysis.health_alerts.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8">
                    <div className="flex items-center gap-3 mb-6">
                    ...
                    <div className="space-y-4">
                      {parsedAnalysis.health_alerts.map((alert, index) => {
                        ...
                        return (
                          <div
                            key={index}
                            className={`${severityInfo.bg} border rounded-2xl p-6`}
                          >
                            <div className="flex items-start gap-4">
                              <div
                                className={`w-10 h-10 ${severityInfo.icon} rounded-xl flex items-center justify-center flex-shrink-0`}
                              >
                              ...
                              <div className="flex-1">
                                <div className="flex items-center justify-between mb-3">
                                  ...
                                  <span
                                    className={`px-3 py-1 text-xs font-semibold rounded-full border ${severityInfo.badge}`}
                                  >
              ...
              {parsedAnalysis?.alternatives &&
                parsedAnalysis.alternatives.length > 0 && (
                  <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/20 p-8">
                    <div className="flex items-center gap-3 mb-6">
