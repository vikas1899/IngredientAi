...
import {
  Camera, ChevronLeft, ChevronRight, Search, AlertTriangle, CheckCircle,
  Calendar, Eye, TrendingUp, Shield, XCircle, Sparkles, Target,
  Clock, FileImage, Info, Filter, Trash2, Loader
} from 'lucide-react';
...
const History = () => {
  ...
  const parseAnalysisResult = (result) => {
    if (!result) return { status: 'safe', summary: 'Analysis completed', details: null };
    ...
    if (typeof result === 'string') {
      try {
        ...
        if (resultLower.includes('caution') || resultLower.includes('moderate')) {
    ...
    if (typeof result === 'object') {
      ...
      if (recommendation?.verdict === 'avoid') {
        ...
        status = 'caution';
      } else if (recommendation?.verdict === 'recommend' || recommendation?.verdict === 'safe') {
        status = 'safe';
      }

      return {
        status,
        summary: analysis_summary?.main_verdict || recommendation?.reason || 'Analysis completed',
        details: result,
  ...
  const getStatusBadge = (analysisData) => {
    const { status, safetyScore, concerns, verdict } = analysisData;
  ...
  const filteredAnalyses = analyses.filter(analysis => {
    ...
    return (
      (analysis.category && analysis.category.toLowerCase().includes(searchLower)) ||
      (analysisData.summary && analysisData.summary.toLowerCase().includes(searchLower)) ||
      ...
      <>
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium">Loading your analysis history...</p>
          </div>
        </div>
      </>
    );
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          <header className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
              Review your past ingredient analyses and track your product safety journey over time.
            </p>
          </header>
          ...
          {error && (
            <div className="mb-8 p-6 bg-red-50 border border-red-200 rounded-2xl flex items-center gap-4 shadow-lg">
              ...
              <div>
                <h4 className="font-semibold text-red-800">Error Loading History</h4>
            ...
            {(searchTerm || filterStatus !== 'all') && (
              <div className="mt-4 flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  Found {filteredAnalyses.length} result{filteredAnalyses.length !== 1 ? 's' : ''}
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-lg border border-white/20 p-12 text-center">
              <div className="max-w-md mx-auto">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-4">
                  {searchTerm || filterStatus !== 'all' ? 'No matching analyses found' : 'No analyses found'}
                </h3>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          ></div>
          <div className={`bg-white/95 rounded-2xl p-8 max-w-md mx-4 shadow-2xl relative z-10 ${showDeleteConfirm ? 'animate-fade-in' : ''}`}>
            <div className="text-center">
